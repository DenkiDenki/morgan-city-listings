...
function applyFilters(homes){
	return homes.filter( home => {
		...
		if( availableLotSwitch && home.custom_fields["const-status_rel"].slug === 'available-lot'){
		...
		if( underConstructionSwitch && home.custom_fields["const-status_rel"].slug === 'under-construction'){
		...
		if( moveInReadySwitch && home.custom_fields["const-status_rel"].slug === 'move-in-ready'){
    	...
    	//Price filter
      	...
      	// State filter
    	if ( document.querySelector('input[name="state"]:checked').value === home.custom_fields.state.slug){
			return true; // include it
		} else {
			return false; // exclude it
		}
...
function updateMarkers(){
  ...
  mapMarkersV2 = applyFilters(homesWithLatAndLong).map( function( home ){
    let marker = {
      ...
      title: home.title,
      image: home.custom_fields.gallery.length && home.custom_fields.gallery[0].sizes && home.custom_fields.gallery[0].sizes.list_img_crop ? home.custom_fields.gallery[0].sizes.list_img_crop : '/wp-content/uploads/2020/06/no-image.jpg',
      ...
      relatedFloorplans: home.floorplans.map(f => f.tiny_image ? f : Object.assign(f, {tiny_image: '/wp-content/uploads/2020/06/no-image.jpg'})),
...
function getInfoWindowCustomTemplate( home ){
  if ( home.custom_fields["const-status_rel"].slug !== 'available-lot' ) {
